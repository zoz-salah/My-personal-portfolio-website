...
// ─── CUSTOM CURSOR (smooth trailing dot) – DISABLED ON TOUCH DEVICES ───
(function() {
  // Check if the device has a coarse pointer (touch screen) or cannot hover
  const isTouchDevice = () => {
  ...
  if (isTouchDevice() || prefersReducedMotion.matches) {
    // Exit immediately – no custom cursor on touch devices
    return;
  }

  // Configuration (edit these values as you like)
  const CONFIG = {
    color: 'rgba(200, 241, 53, 0.35)',   // semi‑transparent accent
    dotSize: 8,                          // radius in pixels
    lag: 12,                             // higher = more trailing
    zIndex: 9999
  };
